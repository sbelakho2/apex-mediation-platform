...
export default function PlacementDetailPage({ params }: { params: { id?: string } }) {
  ...
  // Basic validation: non-empty, up to a sensible length; real validation happens via fetch too
  if (!id || id.length > 128) {
    return notFound();
  }
...
// Using a client component keeps credentials/cookies behavior aligned with the shared api client
'use client';
...
function PlacementDetailClient({ id }: { id: string }) {
  ...
  async function toggleActive() {
    ...
    const res = await api.put(`/api/v1/placements/${placement.id}`, { status: next }, {
