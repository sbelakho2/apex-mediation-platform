...
// Client component for data fetching and interactive actions
'use client';
...
function AppDetailClient({ id }: { id: string }) {
  ...
  async function toggleEnabled() {
    ...
    const res = await api.put(`/api/v1/apps/${encodeURIComponent(appConfig.id)}`, { enabled: next }, {
  ...
  async function rotateKey() {
    ...
    setSaving(true);
    setError(null);
    const res = await api.post(`/api/v1/apps/${encodeURIComponent(appConfig.id)}/rotate-key`, {}, {
      headers: { ...(csrf ? { 'X-CSRF-Token': csrf } : {}) },
    });
    if (!res.success || !res.data) {
      setError(res.error || 'Failed to rotate SDK key');
    } else {
      // Expect either data.sdkKey or nested data
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="mt-2 text-sm text-gray-600 flex gap-3">
                  <span>Platform: <strong className="text-primary-blue">{appConfig.platform.toUpperCase()}</strong></span>
                  {appConfig.bundleId && <span>Bundle ID: <strong className="text-primary-blue">{appConfig.bundleId}</strong></span>}
                  {appConfig.packageName && <span>Package: <strong className="text-primary-blue">{appConfig.packageName}</strong></span>}
                </div>
              </div>
