(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.RivalApexWebSDK = {}));
    ...
    class Emitter {
        ...
        emit(event, payload) {
            var _a;
            (_a = this.handlers.get(event)) === null || _a === void 0 ? void 0 : _a.forEach((h) => {
                try {
                    h(payload);
                }
                catch {
    ...
    const Errors = {
        ...
        invalidOptions: (msg, details) => new SdkError('INVALID_OPTIONS', msg, details),
        network: (msg, details, cause) => new SdkError('NETWORK', msg, details, cause),
        timeout: (timeoutMs) => new SdkError('TIMEOUT', `Request timed out after ${timeoutMs}ms`, { timeoutMs }),
        badResponse: (status, body) => new SdkError('BAD_RESPONSE', `Unexpected response status ${status}`, { status, body }),
        validation: (msg, details) => new SdkError('VALIDATION', msg, details),
        noFill: () => new SdkError('NO_FILL', 'No ad fill returned from auction'),
        unknown: (msg = 'Unknown SDK error', details, cause) => new SdkError('UNKNOWN', msg, details, cause),
    };
    ...
    const ConsentSchema = objectType({
        gdprApplies: booleanType().optional(),
        tcfConsent: stringType().min(1).optional().nullable(),
        usPrivacy: stringType().min(1).optional().nullable(),
        gpp: stringType().min(1).optional().nullable(),
        coppa: booleanType().optional(),
    });
    const AdRequestSchema = objectType({
        placement: stringType().min(1),
        adType: enumType(['banner', 'interstitial', 'rewarded']),
        width: numberType().int().positive().optional(),
        height: numberType().int().positive().optional(),
    ...
    objectType({
        ...
        meta: objectType({
            sdk: objectType({ name: stringType(), version: stringType() }),
            publisherId: stringType().optional(),
            appId: stringType().optional(),
        }),
    });
    const AdCreativeSchema = objectType({
        id: stringType(),
        html: stringType().optional(),
        vastTagUrl: stringType().url().optional(),
        tracking: recordType(stringType()).optional(),
    });
    const AdResponseSchema = objectType({
        requestId: stringType(),
        fill: booleanType(),
        price: numberType().optional(),
        currency: stringType().optional(),
        creative: AdCreativeSchema.nullable().optional(),
        ttlSeconds: numberType().int().positive().optional(),
    });
    ...
    async function requestAuction(cfg, consent, req) {
        var _a, _b, _c;
            ...
            while (true) {
                ...
                try {
                    const res = await fetch(endpoint, {
                        ...
                        body: JSON.stringify({
                            request: req,
                            consent: consent !== null && consent !== void 0 ? consent : {},
                            meta: {
                    ...
                    if (!res.ok) {
                        ...
                        if (isRetryableStatus(res.status) && attempt <= ((_a = cfg.maxRetries) !== null && _a !== void 0 ? _a : 0)) {
                    ...
                    if (nameStr.includes('abort')) {
                        ...
                        if (attempt <= ((_b = cfg.maxRetries) !== null && _b !== void 0 ? _b : 0)) {
                            const backoff = cfg.retryBackoffBaseMs * Math.pow(2, attempt - 1);
                            const jitter = (Math.random() * (cfg.retryJitterMs * 2)) - cfg.retryJitterMs;
                            await sleep(Math.max(0, Math.round(backoff + jitter)), globalController.signal);
                            continue;
                    ...
                    if (attempt <= ((_c = cfg.maxRetries) !== null && _c !== void 0 ? _c : 0)) {
                        const backoff = cfg.retryBackoffBaseMs * Math.pow(2, attempt - 1);
                        const jitter = (Math.random() * (cfg.retryJitterMs * 2)) - cfg.retryJitterMs;
                        await sleep(Math.max(0, Math.round(backoff + jitter)), globalController.signal);
                        continue;
                    ...
                    globalController.signal.removeEventListener('abort', onAbort);
                }
            }
        }
        catch (e) {
     ...
     */
    function buildCanonicalConfigJson(config) {
        const sortedMap = {};
     ...
     */
    async function validateConfigHash(config, serverHash) {
     ...
     */
    async function fetchConfigWithHash(endpoint, appId) {
    ...
    function createDefaultConfig(clientConfig) {
        var _a;
        return {
            appId: (_a = clientConfig.appId) !== null && _a !== void 0 ? _a : '',
            version: 1,
            floors: {},
            caps: {},
            pacing: {
    ...
    function init(options) {
        ...
        const endpoint = (_a = options === null || options === void 0 ? void 0 : options.endpoint) === null || _a === void 0 ? void 0 : _a.trim();
        if (!endpoint)
            throw Errors.invalidOptions('endpoint is required');
        const timeoutMs = Math.max(1, (_b = options.timeoutMs) !== null && _b !== void 0 ? _b : 2000);
        const maxRetries = Math.max(0, (_c = options.maxRetries) !== null && _c !== void 0 ? _c : 2);
        const retryBackoffBaseMs = Math.max(0, (_d = options.retryBackoffBaseMs) !== null && _d !== void 0 ? _d : 100);
        const retryJitterMs = Math.max(0, (_e = options.retryJitterMs) !== null && _e !== void 0 ? _e : 50);
        state.cfg = {
            endpoint,
            timeoutMs,
            publisherId: options.publisherId,
            appId: options.appId,
            sdkVersion: (_f = options.sdkVersion) !== null && _f !== void 0 ? _f : '0.1.0',
            maxRetries,
            retryBackoffBaseMs,
            retryJitterMs,
        };
    ...
    function setConsent(consent) {
        ...
        if (!parsed.success)
            throw Errors.validation('Invalid consent object', { issues: parsed.error.issues });
        state.consent = parsed.data;
        emitter.emit('consent:updated', { consent: state.consent });
    }
    async function requestAd(req) {
        if (!state.initialized || !state.cfg)
            throw Errors.initRequired();
        const parsed = AdRequestSchema.safeParse(req);
        if (!parsed.success)
            throw Errors.validation('Invalid ad request', { issues: parsed.error.issues });
        ...
        try {
            ...
            const err = e;
            emitter.emit('ad:error', { error: { code: err.code, message: err.message } });
            throw e;
        }
    }
    ...
    exports.fetchConfigWithHash = fetchConfigWithHash;
    exports.getSupportedAdapters = getSupportedAdapters;
    exports.init = init;
    exports.on = on;
    exports.requestAd = requestAd;
    exports.setConsent = setConsent;
    exports.validateConfigHash = validateConfigHash;
