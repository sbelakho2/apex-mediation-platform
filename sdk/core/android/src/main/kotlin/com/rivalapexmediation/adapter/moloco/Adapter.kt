package com.rivalapexmediation.adapter.moloco
...
class Adapter : AdNetworkAdapter {
    override val networkName: String = "moloco"
    ...
    override fun initialize(context: Context, config: Map<String, Any>) {
        val seatId = config["seat_id"] as? String
            ?: throw IllegalArgumentException("seat_id required")
        val apiKey = config["api_key"] as? String
            ?: throw IllegalArgumentException("api_key required")
        initialized = seatId.isNotBlank() && apiKey.isNotBlank()
    }
    ...
    override fun loadAd(
        ...
        if (!supportsAdType(adType)) {
            callback.onAdFailed("Unsupported ad type", -2)
            return
        }
        val ad = Ad(
            ...
            adType = adType,
            ecpm = 2.5,
            creative = Creative.Banner(width = 320, height = 50, markupHtml = "<div>Moloco Mock</div>")
        )
