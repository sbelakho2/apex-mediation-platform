...
/// Ad source with priority and weight configuration for CTV
public struct WeightedAdSource {
    ...
    public init(
        ...
        priority: Int,
        weight: Double = 1.0,
        timeout: TimeInterval = 8.0, // Longer default for CTV
        enabled: Bool = true,
        minBid: Double = 0.0,
        adFormat: String? = nil,
...
public struct PriorityWeightedConfig {
    ...
    public init(
        ...
        maxConcurrentRequests: Int = 2, // Lower for CTV
...
/// Priority-weighted mediation manager for CTV
public final class PriorityWeightedMediation {
    ...
    public func getPerformanceStats() -> [SourcePerformance] {
        ...
        return sourceStats.map { (sourceId, stats) in
            SourcePerformance(
                ...
                averageLatencyMs: stats.successes > 0 ? Double(stats.totalLatencyMs) / Double(stats.successes) : 0,
                fillRate: stats.attempts > 0 ? Double(stats.successes) / Double(stats.attempts) : 0,
                averageBid: stats.successes > 0 ? Double(stats.totalBidMicros) / Double(stats.successes * 1_000_000) : 0
            )
        }
    }
    ...
    private func selectByWeight(_ sources: [WeightedAdSource]) -> WeightedAdSource {
        guard sources.count > 1 else { return sources[0] }
        
        let effectiveWeights = sources.map { ($0, calculateEffectiveWeight($0)) }
        let totalWeight = effectiveWeights.reduce(0) { $0 + $1.1 }
        guard totalWeight > 0 else { return sources.randomElement()! }
    ...
    private func calculateEffectiveWeight(_ source: WeightedAdSource) -> Double {
        guard config.usePerformanceWeighting else { return source.weight }
    ...
    private func executeWithTimeout(
        ...
        return await withTaskGroup(of: AdLoadResult?.self) { group in
            group.addTask {
                do { return try await loader(source) }
                catch { return .error(sourceId: source.id, error: error) }
            }
            ...
            if let result = await group.next() {
                group.cancelAll()
                return result ?? .timeout(sourceId: source.id)
            }
    ...
    private func calculateAdaptiveTimeout(_ source: WeightedAdSource) -> TimeInterval {
        ...
        return min(source.timeout, max(2.0, avgLatencyMs * 2 / 1000))
    }
    ...
    private func recordResult(_ sourceId: String, result: AdLoadResult) {
        ...
        case .noFill: stats.noFills += 1
        case .error: stats.errors += 1
        case .timeout: stats.timeouts += 1
        }
