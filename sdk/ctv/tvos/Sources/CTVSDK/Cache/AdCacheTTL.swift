...
/// AdCacheTTL - Computes ad cache TTL based on refresh interval for tvOS.
///
...
public enum AdCacheTTL {
    ...
    /// Default refresh interval in seconds for CTV (longer than mobile)
    ...
    ///   - podDurationSeconds: The total duration of the ad pod (optional)
    ...
    public static func calculateEffectiveTTL(
        ...
        podDurationSeconds: TimeInterval? = nil,
        serverTTLSeconds: TimeInterval? = nil
    ) -> TimeInterval {
        var baseTTL = calculateTTL(refreshIntervalSeconds: refreshIntervalSeconds)
        
        // For video pods, ensure TTL covers the pod duration plus buffer
        if let podDuration = podDurationSeconds, podDuration > 0 {
            let podBasedTTL = podDuration + 30 // 30-second buffer
            baseTTL = max(baseTTL, podBasedTTL)
        }
        
        // Respect server TTL if provided
        if let serverTTL = serverTTLSeconds, serverTTL > 0 {
            baseTTL = min(baseTTL, serverTTL)
        }
        
        return min(baseTTL, maxTTLSeconds)
    }
    ...
    public static func shouldPrefetch(
        ...
        // to account for longer video ad load times
        let prefetchThreshold = refreshIntervalSeconds * 1.5
        return remaining > 0 && remaining < prefetchThreshold
    }
}

/// Metadata for a cached CTV ad entry with TTL information.
public struct AdCacheMetadata {
    ...
    public let podDurationSeconds: TimeInterval?
    ...
    public init(
        ...
        self.podDurationSeconds = podDurationSeconds
    }
