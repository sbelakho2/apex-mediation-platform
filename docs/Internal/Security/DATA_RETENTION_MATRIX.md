...
| Transparency receipts (`postgres.transparency_auctions`, `postgres.transparency_auction_candidates`) | 365 days | 48 hours (GDPR delete) | Postgres daily partitions + staging merges (`20251201_134500_partition_transparency_tables.up.sql`, `20251201_135600_transparency_staging_tables.up.sql`) enforce retention; deletion worker replays hash references to redact request IDs. | Privacy Eng | `docs/Transparency/VERIFY.md`, `backend/migrations/postgres/20251201_134500_partition_transparency_tables.up.sql`, `backend/migrations/postgres/20251201_135600_transparency_staging_tables.up.sql` |
| Auction logs (`postgres.analytics_bid_landscape`, Loki traces) | 30 days raw, 180 days aggregated | 72 hours | Postgres partition detach/export defined in `20251202_094500_bid_landscape_tables.up.sql` plus Loki retention config (`monitoring/loki-config.yml`, `monitoring/prometheus.yml`). | SRE | `backend/migrations/postgres/20251202_094500_bid_landscape_tables.up.sql`, `monitoring/loki-config.yml`, `monitoring/prometheus.yml` |
| SDK telemetry + paid events (`analytics_sdk_telemetry`, `analytics_revenue_events`) | 90 days raw, 1 year aggregated | 7 days | Postgres partitioning + rollups (`20251201_131500_partition_analytics_tables.up.sql`, `20251201_150500_analytics_rollup_tables.up.sql`) with redaction workflow documented in `docs/Internal/Development/ANALYTICS_IMPLEMENTATION.md`. | Data Eng | `backend/migrations/postgres/20251201_131500_partition_analytics_tables.up.sql`, `backend/migrations/postgres/20251201_150500_analytics_rollup_tables.up.sql`, `docs/Internal/Development/ANALYTICS_IMPLEMENTATION.md` |
...
2. **Privacy worker** enqueues the delete job, removing PII from Postgres and requesting S3 object deletes where applicable.
