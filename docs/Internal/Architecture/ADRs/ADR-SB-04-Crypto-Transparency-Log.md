...
## Decision
- **Storage:** Postgres tables `transparency_receipts` and `transparency_signer_keys` (see `backend/migrations/025_transparency_receipts.sql` and `backend/migrations/026_ed25519_keys.sql`; legacy ClickHouse DDL remains under `backend/migrations/clickhouse/`) hold canonical payloads plus Ed25519 signatures.
...
- Breaking the signer (e.g., Postgres outage) triggers breaker + PagerDuty alerts, ensuring availability.
