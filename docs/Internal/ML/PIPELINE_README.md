...
- GPU orchestration path via `ML/scripts/nightly_pipeline_gpu.py` that streams the public datasets,
  engineers features with Dask, and trains a GPU-accelerated gradient boosted model (requires
  RAPIDS cuML or XGBoost with CUDA).
...
Optional extras
- Some downstream jobs (e.g., AWS Glue/Spark conversions) still expect `pyarrow`. Install the fastparquet-only base stack first as shown above, then add the extra when working in an environment that ships compatible wheels:
  - `pip install -e "ML[glue]"`
  - Python ≥3.13 typically needs the latest `pyarrow` (wheels may lag platform releases); for older runtimes the extra pins to a stable 16.x line. Consult the change log in `DEVELOPMENT_TODO_CHECKLIST.md` for the most recent compatibility notes.
- GPU path: install `pip install -r ML/requirements-gpu.txt` (adds PyTorch, TabPFN, and XGBoost). RAPIDS/cuML wheels must be installed separately per NVIDIA guidance.
...
- `models/fraud/dev/<date>/weak_supervision/quality_report.json` — nightly weak label coverage, conflict rate, and precision proxies (auto-generated by cron)
- GPU pipeline outputs land under `models/fraud/gpu_workdir/`:
  - `features/features.parquet` + `features/feature_manifest.json`
  - `model/trained_fraud_model.json`, `model/evaluation_metrics.json`, and the serialized GPU model artifact
  - Acceptance checks (ROC AUC ≥ 0.85, Precision ≥ 0.8 @ Recall ≥ 0.9) enforced by `train_gbm_gpu.py`
...
- This is PR1 (Foundations) in the ML Part 3 plan. Next steps:
  ...
  - Fold nightly weak supervision quality reports into monitoring dashboards for ongoing drift checks.
