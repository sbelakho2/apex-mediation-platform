...
jest.mock('../../queues/queueManager', () => {
  ...
  return {
    ...actual,
    queueManager: {
      addJob: jest.fn(async () => ({ id: 'job_123' })),
    },
...
describe('POST /privacy/export', () => {
  ...
  it('enqueues export job and returns job id', async () => {
    const res = await request(app).post('/privacy/export').send({ format: 'json' })
  ...
  it('validates tenant/user context', async () => {
    ...
    const res = await request(app2).post('/privacy/export').send({})
