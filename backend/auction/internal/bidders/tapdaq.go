...
// TapdaqAdapter implements a minimal S2S bidder for Tapdaq with standardized resiliency
// and observability, supporting offline conformance via test_endpoint.
type TapdaqAdapter struct {
	appID  string
	apiKey string
	client *http.Client
	cb     *CircuitBreaker
...
func NewTapdaqAdapter(appID, apiKey string) *TapdaqAdapter {
	return &TapdaqAdapter{
		appID:  appID,
		apiKey: apiKey,
...
func (a *TapdaqAdapter) GetName() string { return "tapdaq" }
func (a *TapdaqAdapter) GetTimeout() time.Duration { return 5 * time.Second }

func (a *TapdaqAdapter) RequestBid(ctx context.Context, req BidRequest) (*BidResponse, error) {
	...
	endpoint := "https://api.tapdaq.com/s2s/bid"
