...
// VungleAdapter implements a minimal S2S bidder for Vungle with standardized resiliency
...
func NewVungleAdapter(appID, apiKey string) *VungleAdapter {
...
func (a *VungleAdapter) GetName() string { return "vungle" }
func (a *VungleAdapter) GetTimeout() time.Duration { return 5 * time.Second }

func (a *VungleAdapter) RequestBid(ctx context.Context, req BidRequest) (*BidResponse, error) {
	...
	endpoint := "https://api.vungle.com/s2s/bid"
	...
	operation := func() error {
		...
		// Use Bearer token for Vungle-style auth (placeholder for dev)
