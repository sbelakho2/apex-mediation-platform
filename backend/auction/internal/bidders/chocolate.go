...
// ChocolateAdapter implements a minimal S2S bidder for Chocolate Platform with standardized resiliency
// and observability hooks. This is implemented to support offline conformance via test_endpoint.
type ChocolateAdapter struct {
	appID  string
	apiKey string
	client *http.Client
	cb     *CircuitBreaker
...
func NewChocolateAdapter(appID, apiKey string) *ChocolateAdapter {
	return &ChocolateAdapter{
		appID:  appID,
		apiKey: apiKey,
...
func (a *ChocolateAdapter) GetName() string { return "chocolate" }
func (a *ChocolateAdapter) GetTimeout() time.Duration { return 5 * time.Second }

func (a *ChocolateAdapter) RequestBid(ctx context.Context, req BidRequest) (*BidResponse, error) {
  ...
  CaptureDebugEventWithSpan(span, DebugEvent{PlacementID: req.PlacementID, RequestID: req.RequestID, Adapter: a.GetName(), Outcome: "no_bid", Reason: NoBidCircuitOpen, CreatedAt: time.Now()})
	...
	endpoint := "https://api.chocolateplatform.com/s2s/bid"
	...
	// Parse a minimal winner structure common in conformance tests
	ad := map[string]any{}
